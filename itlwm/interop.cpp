...
void itlwm::associate(uint8_t *ssid, uint32_t ssid_len, uint32_t authtype_lower, uint32_t authtype_upper, uint8_t *key, uint32_t key_len, int key_index) {
	struct ieee80211com *ic = &com.sc_ic;
	...
	if (authtype_upper == APPLE80211_AUTHTYPE_NONE && authtype_lower == APPLE80211_AUTHTYPE_OPEN) {
...
IOReturn itlwm::bgscan(uint8_t* channels, uint32_t length, const char* ssid, uint32_t ssid_len) {
	...
	if (ic->ic_state == IEEE80211_S_RUN) {
		if (length == 0) {
			...
			setbit(ic->ic_chan_scan_target, channels[ieee80211_chan2ieee(ic, ic->ic_bss->ni_chan)]);
		}
